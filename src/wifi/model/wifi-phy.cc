    ...
    case WIFI_MOD_CLASS_ERP_OFDM:
      {
        ...
        // std::cout <<"wifi ofdmrate: " <<  payloadMode.GetDataRate (txVector.GetChannelWidth (), 0, 1) << std::endl;
        double numSymbols;
    ...
    case WIFI_MOD_CLASS_VHT:
      {
        ...
        double numSymbols;
        // std::cout << "wifi datarate (raw) : " << payloadMode.GetDataRate (txVector.GetChannelWidth (), txVector.IsShortGuardInterval (), txVector.GetNss ()) <<std::endl;
        if (mpdutype == MPDU_IN_AGGREGATE && preamble != WIFI_PREAMBLE_NONE)
          {
            //First packet in an A-MPDU
